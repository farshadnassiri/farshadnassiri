...
/**
 * برچسب سه‌راهه کارت «تفاضل در قیمت فعلی» تب رول — همان سه‌راهه‌ای که
 * core/positions.mjs در rollAnalysis().verdict حساب می‌کند (>0 / <0 / صفر
 * یا NaN)، اینجا کوتاه‌شده برای کارت KPI. مقایسه ساده «> 0» تساوی دقیق
 * (رول بدون تفاوت) و NaN (تحلیل ناقص) را هر دو «نگه داشتن بهتر است»
 * می‌گرفت — نتیجه‌گیری کاذب به‌جای خنثی (دور ۶۶).
 */
export function rollVerdictLabel(atSpot) {
  if (atSpot > 0) return 'رول بهتر است';
  if (atSpot < 0) return 'نگه داشتن بهتر است';
  return 'تفاوتی ندارد';
}
