...
export function generateCombos(def, ua, s, funnel = emptyFunnel()) {
  // «قیمت پایه» زنده باید آخرین معامله را ترجیح بدهد، نه پایانی — همان
  // قرارداد chain.mjs (underlyingQuote/underlyingList). قبلاً همین دو
  // خط، هم spot (→ ctx.S، پنجره ترکیب، بازده ایستا، احتمال سود، سربه‌سری)
  // و هم spotClose (→ ctx.Sclose، وجه تضمین/یونانی، عمداً پایانی-ترجیح) را
  // با فرمول یکسان «پایانی سپس آخرین» می‌ساختند — یعنی S و Sclose همیشه
  // یک عدد می‌شدند و در نوسان روز، «قیمت پایه» نمایشی از آخرین معامله
  // واقعی عقب می‌ماند.
