...
group('۱. بلک-شولز و یونانی');
{
  ...
  // باگ بک‌لاگ #۷: تنصیف خالص برای رسیدن به این دقت به ~۲۰ قدم نیاز داشت؛
  // نیوتن روی وگا با تنصیف به‌عنوان تور ایمنی، چند برابر سریع‌تر همگرا
  // می‌شود. سقف قدم را عمداً پایین می‌گیریم تا اگر کسی نیوتن را برداشت،
  // این آزمون با همان دقتی که تنصیف تنها در ۸ قدم می‌دهد شکست بخورد.
  {
    const mktC = bsPrice('call', 100, 100, 0.25, 0.3, 0.05, 0.6);
    const ivC = impliedVol('call', mktC, 100, 100, 0.25, 0.3, 0.05, { iters: 8 });
    check('تلاطم ضمنی در ۸ قدم همگرا می‌شود — نیوتن، نه فقط تنصیف',
      near(ivC, 0.6, 1e-4), `${ivC.toFixed(6)}`);

    const mktP = bsPrice('put', 100, 70, 0.5, 0.3, 0.05, 0.4);
    const ivP = impliedVol('put', mktP, 100, 70, 0.5, 0.3, 0.05, { iters: 8 });
    check('همگرایی سریع برای پوت دور از پول هم برقرار است',
      near(ivP, 0.4, 1e-4), `${ivP.toFixed(6)}`);
  }
...
group('۴. وجه تضمین — شش مشاهده تابلو');
{
  const BOARD = [
    ...
    // پریمیوم ۱٬۸۶۱ است، نه ۱٬۹۰۱ که در برداشت اول ثبت شده بود — بک‌لاگ #۱۲.
    // با ۱٬۹۰۱، اتحاد ۴۰٬۰۰۰ کم می‌آورد (۷٬۱۰۱٬۰۰۰ در برابر تابلو ۷٬۰۶۱٬۰۰۰).
    // با ۱٬۸۶۱، اتحاد دقیقاً صفر خطا می‌دهد: ۵٬۲۰۰٬۰۰۰ + ۱٬۸۶۱×۱٬۰۰۰ = ۷٬۰۶۱٬۰۰۰.
    // موتور تغییری نکرد؛ عدد قدیمی یک خطای رونویسی در ثبت اولیه بوده.
    { name: 'ضهرم5033', ua: 'اهرم', snap: 'A', kind: 'call', K: 46000, size: 1000, prem: 1861, im: 5200000, rm: 7061000 },
  ...
  check('اتحاد RM = IM + پریمیوم × اندازه، در هر شش مشاهده', idOk === 6, `${idOk} از ۶`);
...
group('۱۶. موتور چند-سررسیدی — کرانداری');
{
  ...
  // باگ بک‌لاگ #۴: بازه نمونه‌برداری regions از ۰٫۳۵×پایه شروع می‌شد. مورب
  // پوت با قیمت اعمال خیلی پایین‌تر از آن (اینجا ۲۰٬۰۰۰ و ۲۵٬۰۰۰ در برابر
  // پایه ۱۰۰٬۰۰۰، یعنی کف قدیمی ۳۵٬۰۰۰) یک ناحیه سود کامل زیر آن کف دارد —
  // با نمونه‌برداری قدیمی، اولین نمونه (سر ۳۵٬۰۰۰) از قبل زیان‌ده بود، پس کل
  // ناحیه سود هرگز دیده نمی‌شد و احتمال سود صفر محاسبه می‌شد.
  const putLeg = (side, K, price, days, ratio = 1) =>
    ({ kind: 'put', side, ratio, strike: K, price, size, days, sigma: 0.6 });
  const lowPutDiag = [putLeg('sell', 20000, 200, 20), putLeg('buy', 25000, 900, 80)];
  const d = run(lowPutDiag);
  check('زیر کف قدیمی نمونه‌برداری، تابع هنوز زیان‌ده است — آزمون خودش شاهد باگ است',
    d.at(35000) < 0, `${Math.round(d.at(35000)).toLocaleString()}`);
  check('ناحیه سود زیر کف قدیمی دیده می‌شود، نه بریده',
    d.regions.length > 0 && d.regions[0][0] < 35000 && d.regions[0][1] <= 35000 + 1e-6,
    d.regions.map(([lo2, hi2]) => `${lo2.toFixed(1)}..${hi2.toFixed(1)}`).join(' , ') || 'بدون ناحیه');
}
