...
// نیوتن روی وگا با تنصیف به‌عنوان تور ایمنی. اینجا دقیقاً همان جاهایی که
// وگا نزدیک صفر است و نیوتن تنها می‌تواند واگرا شود — عمیقاً درون یا بیرون
// پول، با سررسید خیلی کوتاه یا خیلی بلند — سنجیده می‌شود.
group('نیوتن روی وگا — رفت و برگشت در نقاط پرخطر وگای صفر');
{
  const cases = [
    { S: 100000, K: 70000, T: 0.1, sig: 0.4, label: 'درون پول، سررسید کوتاه' },
    { S: 100000, K: 140000, T: 0.1, sig: 0.4, label: 'بیرون پول، سررسید کوتاه' },
    { S: 100000, K: 100000, T: 3, sig: 1.2, label: 'اعمال برابر، سررسید بلند و تلاطم بالا' },
    { S: 100000, K: 100000, T: 0.01, sig: 0.5, label: 'اعمال برابر، سررسید خیلی کوتاه' },
    { S: 100000, K: 95000, T: 1, sig: 0.15, label: 'تلاطم پایین' },
  ];
  for (const { S, K, T, sig, label } of cases) {
    for (const kind of ['call', 'put']) {
      const mkt = bsPrice(kind, S, K, T, 0.3, 0, sig);
      const iv = impliedVol(kind, mkt, S, K, T, 0.3, 0, { lo: 0.001, hi: 5 });
      check(`${label} — ${kind}`, near(iv, sig, 1e-3), `${iv} ~ ${sig}`);
    }
  }
}
...
{
  // بک‌لاگ ۱۲ — بازبینی مشاهده ضهرم5033: محاسبه ۷٬۱۰۱٬۰۰۰ در برابر تابلو
  // ۷٬۰۶۱٬۰۰۰، اختلاف دقیقاً ۴۰٬۰۰۰ (۴۰ ریال × اندازه ۱۰۰۰).
  //
  // نتیجه بازبینی: نه گرد کردن، نه ضریب گم‌شده. IM این ردیف (۵٬۲۰۰٬۰۰۰)
  // مستقل تأیید می‌شود — بازه S سازگارش (۴۲٬۶۵۸ تا ۴۲٬۶۶۷) دقیقاً زیرمجموعه
  // بازه ضهرم5034 هم‌برداشت است (۴۲٬۶۰۰ تا ۴۲٬۷۰۰)، پس IM قابل اعتماد است.
  // با اصلاح تک‌فیلدی یا پریمیوم (۱۹۰۱ ← ۱۸۶۱) یا IM (۵٬۲۰۰٬۰۰۰ ← ۵٬۱۶۰٬۰۰۰)،
  // اتحاد RM=IM+پریمیوم×اندازه دقیقاً برقرار می‌شود — هر دو به‌تنهایی کافی‌اند.
  // یعنی فرمول درست است؛ یکی از دو عدد در برداشت از تابلو اشتباه ثبت شده،
  // به‌احتمال زیاد لغزش یک رقم. کدام رقم درست است بدون عکس اصلی تابلو
  // معلوم نیست، پس عدد دست‌کاری نشد — رگرسیون ادعای اجرا صادق می‌ماند.
  // این ردیف با هدف تعمدی نگه داشته شد تا اختلاف واقعی داده خام همیشه
  // آشکار بماند، نه با ضریب اصلاحی پنهان شود.
  const BOARD = [
